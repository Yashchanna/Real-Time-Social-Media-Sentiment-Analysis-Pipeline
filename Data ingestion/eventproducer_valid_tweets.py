...
CONNECTION_STRING = (
    ...
    "EntityPath=valid-reviews-hub"
)

EVENT_HUB_NAME = "valid-reviews-hub"
...
CSV_PATH = r"C:\Real_Time_Sentiment_Project\bronze_valid_reviews_raw.csv"

df = pd.read_csv(CSV_PATH)

print(f"Loaded {len(df)} records from Bronze CSV.")
print(f"Detected schema: {list(df.columns)}")
print("Starting Valid Reviews Producer...")
...
for _, row in df.iterrows():
    ...
    # Automatically detect complete schema
    event = row.to_dict()

    # Convert NaN / missing values to None
    event = {
